...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === "template") {
      systemContent =
        "You are an event planning expert. Generate a detailed event template based on the user's description.";
...
Return a JSON object with all event details:
{
  ...
  "special_notes": "string"
      ...
      systemContent =
        "You are an event planning expert. Generate complete event details from a brief description.";
      userPrompt = `Generate complete event details for: ${prompt}

IMPORTANT: Today's date is ${
        new Date().toISOString().split("T")[0]
      }. The event_date MUST be at least 4 days from today and MUST be after October 3rd, 2024.
...
Return a JSON object with all event details filled in with realistic values:
{
  ...
  "special_notes": "string"
      ...
      systemContent =
        "You are an event planning expert. Generate a complete event plan from just an event name.";
