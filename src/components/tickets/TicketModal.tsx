...
export function TicketModal({ open, onOpenChange, ticketData }: TicketModalProps) {
  ...
  const handleDownload = () => {
    ...
    link.download = `ticket-${ticketData?.attendeeName?.replace(/\s+/g, '-')}.png`;
      ...
      <DialogContent className="max-w-md">
        ...
        <div 
          ...
          style={{
            ...
            fontFamily: ticketData?.font_family || 'Arial',
        ...
        >
          ...
          <div className="border-t pt-4 space-y-2">
            ...
            <p><strong>Type:</strong> {ticketData?.groupType}</p>
            {ticketData?.seat && (
              <p><strong>Seat:</strong> {ticketData.seat}</p>
            )}
            <p><strong>Access Code:</strong> {ticketData?.password}</p>
          </div>

          {ticketData?.show_qr_code !== false && (
            <div className="flex justify-center pt-4">
              <QRCodeSVG 
                value={ticketData?.checkInUrl || `${ticketData?.eventData?.id}-${ticketData?.attendeeName}-${ticketData?.password}`}
                size={150}
              />
            </div>
          )}
