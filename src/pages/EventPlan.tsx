    ...
    return (
      <div ref={setNodeRef} style={style}>
        <div className="relative group">
          <div {...attributes} {...listeners} className="absolute top-2 left-2 z-10 cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div ref={setNodeRef} style={style}>
      <Card className="cursor-pointer hover:shadow-lg transition-shadow group relative">
        ...
        <CardContent className="p-6" onClick={() => navigate(card.path)}>
          <h3 className="font-bold text-lg mb-2">{card.icon} {card.title}</h3>
...
export default function EventPlan() {
  ...
  const [cards, setCards] = useState<CardItem[]>([
    { id: 'details', title: 'Details', icon: '📝', description: 'View and edit event details', path: `/event/${eventId}/details`, component: 'details' },
    { id: 'planner', title: 'Planner', icon: '📅', description: 'Interactive planner with timeline views', path: `/event/${eventId}/plan` },
    { id: 'full-plan', title: 'Full Plan', icon: '📋', description: 'View complete event overview', path: `/event/${eventId}/full-plan` },
    { id: 'tasks', title: 'Tasks', icon: '✅', description: 'Manage event tasks', path: `/event/${eventId}/tasks` },
    { id: 'budget', title: 'Budget', icon: '💰', description: 'Track expenses', path: `/event/${eventId}/budget` },
    { id: 'invites', title: 'Invites', icon: '✉️', description: 'Manage invitations', path: `/event/${eventId}/invites` },
    { id: 'timeline', title: 'Timeline', icon: '⏰', description: 'Event schedule', path: `/event/${eventId}/timeline` },
    { id: 'vendors', title: 'Vendors', icon: '🏪', description: 'Find services & vendors', path: `/event/${eventId}/vendors` },
    { id: 'guests', title: 'Guests', icon: '👥', description: 'Manage attendees & invites', path: `/event/${eventId}/guests` },
    { id: 'tickets', title: 'Tickets', icon: '🎫', description: 'Manage tickets & attendees', path: `/event/${eventId}/manage-tickets` },
    { id: 'food', title: 'Food', icon: '🍽️', description: 'AI food suggestions & recipes', path: `/event/${eventId}/food` },
    { id: 'souvenirs', title: 'Souvenirs', icon: '🎁', description: 'Swag bags & souvenir ideas', path: `/event/${eventId}/souvenirs` },
    { id: 'weather', title: 'Weather', icon: '🌤️', description: 'Weather forecast & tips', path: `/event/${eventId}/weather` },
    { id: 'sponsors', title: 'Sponsors', icon: '🤝', description: 'Find event sponsors', path: `/event/${eventId}/sponsors` },
    { id: 'memories', title: 'Memories', icon: '📸', description: 'AI-generated event memories', path: `/event/${eventId}/memories` },
  ]);
  ...
  const handleAddToAccount = async () => {
    ...
    try {
      ...
      // Copy related data
      const tables = ['tasks', 'budget_items', 'timeline_events', 'invites', 'event_faqs', 'event_speeches'];
